...
function saveExemption() {
    chrome.storage.sync.get(EXEMPTION_CONFIG).then(function(items) {
        ...
        chrome.storage.sync.set(exemptionConfig).then(restoreExemptionTime, onError);
    }, onError);
}
...
function savePrompt() {
    chrome.storage.sync.get(SAVED_PROMPTS).then(function(items) {
        ...
        if (newPrompt != '') {
            ...
            chrome.storage.sync.set(promptConfig).then(restorePrompt, onError);
        }
...
function restorePrompt() {
    chrome.storage.sync.get(SAVED_PROMPTS).then(function(items) {
        ...
        if (items.hasOwnProperty(SAVED_PROMPTS) && items[SAVED_PROMPTS].length > 0) {
            ...
            promptConfig = {}
            promptConfig[SAVED_PROMPTS] = ['Take a walk outside, to refresh yourself.', 'Visit hacker news, to read something interesting.']
            console.log('Updated prompts ' + JSON.stringify(promptConfig))
